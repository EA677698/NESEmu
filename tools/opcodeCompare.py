...
log_path = 'opcodeCount.txt'
more_cycles, less_cycles, total_instructions = parse_log_with_tolerance(log_path)
...
if more_cycles or less_cycles:
    ...
    if more_cycles:
        ...
        for item in more_cycles:
            print(f"Opcode {item['opcode']}: Expected {item['min_cycles']} to {item['max_cycles']} cycles, Logged {item['logged_cycles']} cycles")
...
# Print summary
print(f"\nSummary:")
print(f"Cycle correct: {total_instructions - (len(more_cycles) + len(less_cycles))} / {total_instructions}")
